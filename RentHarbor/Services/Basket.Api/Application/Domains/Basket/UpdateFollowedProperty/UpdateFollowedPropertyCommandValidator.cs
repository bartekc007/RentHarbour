    ...
    public class UpdateFollowedPropertyCommandValidator : AbstractValidator<UpdateFollowedPropertyCommand>
    {
        public UpdateFollowedPropertyCommandValidator()
        {
            RuleFor(x => x.Id)
                ...
                .Length(36).WithMessage("Id must be 36 characters long."); // Assuming GUID length

            RuleFor(x => x.UserId)
                .NotEmpty().WithMessage("UserId is required.")
                .Length(36).WithMessage("UserId must be 36 characters long."); // Assuming GUID length

            RuleFor(x => x.PropertyId)
                .NotEmpty().WithMessage("PropertyId is required.")
                .Length(36).WithMessage("PropertyId must be 36 characters long."); // Assuming GUID length
